...
class Network(object):
    ...
    def __init__(self,config):
        ...
        self.initial_state_label = tf.zeros([self.config.batch_size, self.config.data_sets._len_labels])
    ...
    def projection2(self, rnn_outputs):
    ...
    def projection(self, rnn_outputs):
        ...
        with tf.variable_scope('Projection'):
            ...
            with tf.variable_scope('label'):
                ...
                #U2 = tf.get_variable('Matrix_label', [self.config.mRNN._hidden_size, self.config.data_sets._len_labels])
                #proj_b2 = tf.get_variable('Bias_label', [self.config.data_sets._len_labels])
                # outputs_labels = [tf.matmul(o, U2) + proj_b2 for _,o in rnn_outputs]
                outputs_labels = [o for _,o in rnn_outputs]
    ...
    def predict(self,inputs,keep_prob, _):
        ...
        with tf.variable_scope('RNN') as scope:
            ...
            RNN_H_struc = tf.get_variable('HMatrix_struc',[hidden_size,hidden_size])
